...
## Question 3
### Density plots
#### Consider using densities from rjags model
...
###### Just the densities are enough to answer question 3
...
modeljags <- function(){
  for (i in 1:J){
    Y[i] ~ dbin(p[i], N[i])
    logit(p[i]) <- beta0 + beta1*btn133_400_pc[i] + beta2*great_400pc[i] + beta_location[location[i]]
  }
  #priors
  beta0 ~ dnorm(0, 0.001)
  beta1 ~ dnorm(0, 0.001)
...
params_loc_jags <- c("beta0", "beta1", "beta2", "beta_location")
# Write model to file
...
ggplot(coverage_df_wide, aes(x = location, y = mean, ymin = `lower.2.5%`, ymax = `upper.97.5%`)) +
  geom_pointrange() +
  coord_flip() +
...
mean_coverage_133_400FPL <- mean(plogis(jag7posterior_samples$beta0 + jag7posterior_samples$beta1 + jag7posterior_samples[, grep("^beta_location", colnames(jag7posterior_samples))][, mississippi_index]))
mean_coverage_400FPL <- mean(plogis(jag7posterior_samples$beta0 + jag7posterior_samples$beta2 + jag7posterior_samples[, grep("^beta_location", colnames(jag7posterior_samples))][, mississippi_index]))
mean_coverage_133FPL <- mean(plogis(jag7posterior_samples$beta0 + jag7posterior_samples[, grep("^beta_location", colnames(jag7posterior_samples))][, mississippi_index]))
